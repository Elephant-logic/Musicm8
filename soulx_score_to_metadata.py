...
def merge_rows(rows: list[dict[str, Any]], max_duration_s: float = 9.0, max_gap_s: float = 1.15) -> list[dict[str, Any]]:
    """Join neighbouring lyric lines into short musical phrases.

    One-line generations sounded restart-heavy and synthetic. Two nearby lines can
    share one SoulX inference chunk while remaining comfortably short for a T4.
    """
    if not rows:
        return []
    ...
    def flush() -> None:
        ...
        total = sum(e["duration"] for e in current_events)
        target = max(0.025, (end_ms - start_ms) / 1000.0)
        current_events[-1]["duration"] = max(0.025, current_events[-1]["duration"] + (target - total))
        out.append({
            "index": f"musicm8_{start_ms}_{end_ms}",
            "language": "English",
            "time": [start_ms, end_ms],
            "duration": " ".join(f"{e['duration']:.5f}" for e in current_events),
            "text": " ".join(e["text"] for e in current_events),
            "phoneme": " ".join(e["phoneme"] for e in current_events),
            "note_pitch": " ".join(str(e["pitch"]) for e in current_events),
            "note_type": " ".join(str(e["type"]) for e in current_events),
    ...
    for row in rows:
        ...
        # Usually this gives two lyric lines per chunk; never build long monolithic vocals.
