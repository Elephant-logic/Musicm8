...
def main() -> None:
    p = argparse.ArgumentParser(description="Musicm8 AI producer v10: learned whole-track MIDI composition + sampled instruments + score/phoneme vocals.")
    ...
    # Backwards-compatible old flags. v10 does not put the old inverse-synth engine in the audible path.
    ...
    p.add_argument("--no-ai", action="store_true", help="Use deterministic producer/lyrics fallback; the learned MIDI composer still composes the music.")
    ...
    print("Musicm8 v10 — LEARNED COMPOSER MODE")
    print("  MIDI-LLM composes the complete multitrack score")
    print("  no plan_to_midi rule composer in the audible song")
    print("  one global key transpose only; no per-note harmony rewriting")
    print("  one shared genre swing grid for timing")
    print("  real sampled SoundFont instruments instead of the primitive synth")
    print("  SoulX phrase chunks use words + phonemes + score")
    ...
    print("\n=== 2/12 BUILD REFERENCE LIBRARY (PRODUCTION CONTEXT) ===")
    ...
    print("\n=== 6/12 LEARNED AI COMPOSER — COMPLETE TRACK ===")
    # Deliberately do not call plan_to_midi_v3.py. The notes/rhythms/instrument relations come from MIDI-LLM.
    run([
        sys.executable, "-u", "midi_llm_composer.py",
        "--root", root,
        "--plan", plan,
        "--idea", args.idea,
        "--out", project,
        "--seed", str(args.seed),
        "--candidates", str(max(1, min(4, args.composer_candidates))),
    ], repo)
    if not arrangement.exists():
        raise RuntimeError("Learned composer did not create arrangement.mid")

    print("\n=== 7/12 REAL SAMPLED INSTRUMENT RENDER ===")
    render_cmd = [sys.executable, "-u", "soundfont_render_v1.py", "--project", project, "--plan", plan]
    ...
    if not args.no_vocals:
        print("\n=== 9/12 VOCAL SCORE FOLLOWS THE ACTUAL AI MUSIC ===")
        cmd = [
        ...
        if harmony_midi.exists():
            cmd += ["--harmony-midi", harmony_midi]
        if melody_midi.exists():
            cmd += ["--melody-midi", melody_midi]
        run(cmd, repo)

        print("\n=== 10/12 SOULX CHUNKED SCORE/PHONEME SINGER ===")
        cmd = [
        ...
        if not vocal_ok:
            ...
            print("The exact lyrics are preserved, but Musicm8 will not label an instrumental as a successful vocal song.")
    ...
    payload = {
        "format": "musicm8-ai-project-v10",
        "producer": "Qwen producer brief + genre production director",
        "composer": "MIDI-LLM Llama-3.2-1B learned whole-track text-to-MIDI",
        "composer_report": str(composition_report),
        "engine": "sample-based GM SoundFont multitrack renderer",
        "soundfont_report": str(soundfont_report),
        "mix_engine": "Musicm8 timing-safe stem mix",
        "vocal_engine": "SoulX-Singer phrase chunks + phonemes + actual-arrangement harmony + pitch/word QA" if not args.no_vocals else None,
        "idea": args.idea,
        "seed": args.seed,
        "plan": str(plan),
        ...
        "raw_ai_arrangement": str(project / "arrangement_ai_raw.mid"),
        ...
        "rule_composer_used": False,
        "primitive_synth_used_for_audible_music": False,
    }
    (project / "project.json").write_text(json.dumps(payload, indent=2, ensure_ascii=False), encoding="utf-8")

    print("\n============================================================")
    print("✅ MUSICM8 V10 COMPLETE")
    print("Composer: learned MIDI-LLM whole-track model")
    print("Rule composer used: NO")
    print("Audible primitive synth used: NO")
