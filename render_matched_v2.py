...
def apply_producer_controls(role: str, patch: dict[str, Any], controls: dict[str, Any]) -> dict[str, Any]:
    ...
    if role == "bass":
        for key in ("sub", "drive", "width"):
            if key in controls:
                out[key] = blend(out.get(key, controls[key]), controls[key], 0.36)
        if "filter_motion" in controls:
            motion = clamp(controls["filter_motion"], 0, 1, 0.35)
            out["filter_env"] = blend(out.get("filter_env", 0.3), motion, 0.48)
            out["cutoff"] = float(out.get("cutoff", 1200)) * (0.80 + 0.42 * motion)
    elif role == "drums":
        if "drive" in controls: out["drive"] = blend(out.get("drive", 0.1), controls["drive"], 0.34)
        if "room" in controls: out["room"] = blend(out.get("room", 0.1), controls["room"], 0.34)
        if "brightness" in controls:
            b = clamp(controls["brightness"], 0, 1, 0.5)
            out.setdefault("snare", {})["brightness"] = blend(out.get("snare", {}).get("brightness", 0.5), b, 0.44)
            out.setdefault("hat", {})["highpass"] = float(out.get("hat", {}).get("highpass", 5200)) * (0.80 + 0.46 * b)
    else:
        if "brightness" in controls:
            b = clamp(controls["brightness"], 0, 1, 0.5)
            out["cutoff"] = float(out.get("cutoff", 5000)) * (0.72 + 0.56 * b)
        for key in ("detune", "reverb", "delay", "width"):
            if key in controls: out[key] = blend(out.get(key, controls[key]), controls[key], 0.34)
    return out
...
def fallback_patch(role: str) -> dict[str, Any]:
    ...
    if role == "bass": base.update({"sub": 0.65, "cutoff": 1800, "width": 0.03, "gain": 0.58})
    elif role == "chords": base.update({"detune": 0.18, "reverb": 0.32, "width": 0.60, "gain": 0.30})
    else: base.update({"delay": 0.16, "reverb": 0.24, "width": 0.34, "gain": 0.27})
    return base
...
def render_project(plan_path: Path, midi_path: Path, patches_path: Path, out_dir: Path) -> Path:
    ...
    stems_dir = out_dir / "audio_stems"; stems_dir.mkdir(parents=True, exist_ok=True)
    by_name = {(inst.name or "").lower(): inst for inst in pm.instruments}
    drums_inst = by_name.get("drums")
    rendered: dict[str, np.ndarray] = {}; patch_report: dict[str, Any] = {}
    ...
    prod_mix = plan.get("production", {}).get("mix", {})
    targets = {
        "drums": float(prod_mix.get("drums_db", -16.5)),
        "bass": float(prod_mix.get("bass_db", -19.0)),
        "chords": float(prod_mix.get("chords_db", -23.5)),
        "melody": float(prod_mix.get("melody_db", -24.0)),
    }
    for role, audio in list(rendered.items()): rendered[role] = set_rms(audio, targets.get(role, -22.0), max_gain_db=8.0)

    sc_depth = clamp(prod_mix.get("sidechain_depth", 0.42), 0.0, 0.85, 0.42)
    sc_release = clamp(prod_mix.get("sidechain_release", 0.18), 0.06, 0.6, 0.18)
    sc = sidechain_gain(drums_inst, total_n, depth=sc_depth, release_s=sc_release)
    for role in ("bass", "chords"):
        if role in rendered: rendered[role] = (rendered[role] * sc[None]).astype(np.float32)
    ...
    bus_ratio = clamp(prod_mix.get("bus_ratio", 1.5), 1.0, 3.0, 1.5)
    mix = block_compressor(mix, threshold_db=-10.0, ratio=bus_ratio, makeup_db=0.0)
